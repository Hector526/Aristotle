...
![栈溢出的错误](./img/0c9e2c4f7ee8ca59cfa99a6f51510470.webp)
...
![全局执行上下文](./img/7fa2ed18e702861890d767ea547533da.webp)
...
![函数调用过程](./img/537efd9e96771dc50737117e615533ca.webp)
...
![栈示意图](./img/5e2bb65019053abfd5e7710e41d1b405.webp)
...
![全局执行上下文压栈](./img/a5d7ec1f8f296412acc045835b85431d.webp)
...
![赋值操作改变执行上下文中的值](./img/1d50269dbc5b4c69f83662ecdd977b1d.webp)

接下来，**第二步是调用 addAll 函数**。当调用该函数时，JavaScript 引擎会编译该函数，并为其创建一个执行上下文，最后还将该函数的执行上下文压入栈中，如下图所示：

![执行 addAll 函数时的调用栈](./img/7d6c4c45db4ef9b900678092e6c53652.webp)

addAll 函数的执行上下文创建好之后，便进入了函数代码的执行阶段了，这里先执行的是 d=10 的赋值操作，执行语句会将 addAll 函数执行上下文中的 d 由 undefined 变成了 10。

然后接着往下执行，**第三步，当执行到 add 函数**调用语句时，同样会为其创建执行上下文，并将其压入调用栈，如下图所示：

![执行 add 函数时的调用栈](./img/ccfe41d906040031a7df1e4f1bce5837.webp)

当 add 函数返回时，该函数的执行上下文就会从栈顶弹出，并将 result 的值设置为 add 函数的返回值，也就是 9。如下图所示：

![add 函数执行结束时的调用栈](./img/03ca801a5372f941bf17d6088fee0f96.webp)

紧接着 addAll 执行最后一个相加操作后并返回，addAll 的执行上下文也会从栈顶部弹出，此时调用栈中就只剩下全局上下文了。最终如下图所示：

![addAll 函数执行结束时的调用栈](./img/d0ac1d6e77735338fa97cc9a3f6c717b.webp)
...
![查看函数调用关系](./img/c0d303a289a535b87a6c445ba7f34fa2.webp)
...
![使用 trace 函数输出当前调用栈信息](./img/abfba06cd23a7704a6eb148cff443ece.webp)
...
![栈溢出错误](./img/b4f7196077d9ef4eac1ca6a279f2054d.webp)
