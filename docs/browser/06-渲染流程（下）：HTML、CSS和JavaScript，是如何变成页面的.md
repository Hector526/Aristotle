...
![渲染引擎给页面多图层示意图](./img/e2c917edf5119cddfbec9481372f8fc0.webp)

从上图可以看出，渲染引擎给页面分了很多图层，这些图层按照一定顺序叠加在一起，就形成了最终的页面，你可以参考下图：

![cd6aac705501d48bda6e8eebca058b78](./img/cd6aac705501d48bda6e8eebca058b78.webp)

现在你知道了**浏览器的页面实际上被分成了很多图层，这些图层叠加后合成了最终的页面**。下面我们再来看看这些图层和布局树节点之间的关系，如文中图所示：

![布局树和图层树关系示意图](./img/e8a7e60a2a08e05239456284d2aa4061.webp)
...
![层叠上下文示意图](./img/a03eb12053aac1ac496b61a424f20119.webp)
...
![剪裁执行结果](./img/6a583733735edc1e4d7946740eb6fc0c.webp)

出现这种裁剪情况的时候，渲染引擎会为文字部分单独创建一个层，如果出现滚动条，滚动条也会被提升为单独的层。你可以参考下图：

![7b6ceaab23c6c6d8e5930864ff9d7097](./img/7b6ceaab23c6c6d8e5930864ff9d7097.webp)
...
![绘制列表](./img/40825a55214a7990bba6b9bec6e54108.webp)

从图中可以看出，绘制列表中的指令其实非常简单，就是让其执行一个简单的绘制操作，比如绘制粉色矩形或者黑色的线等。而绘制一个元素通常需要好几条绘制指令，因为每个元素的背景、前景、边框都需要单独的指令去绘制。所以在图层绘制阶段，输出的内容就是这些待绘制列表。

你也可以打开“开发者工具”的“Layers”标签，选择“document”层，来实际体验下绘制列表，如下图所示：

![一个图层的绘制列表](./img/303515c26fcd4eaa9b9966ad7f190370.webp)
...
![渲染进程中的合成线程和主线程](./img/46d33b6e5fca889ecbfab4516c80a441.webp)

如上图所示，当图层的绘制列表准备好之后，主线程会把该绘制列表提交（commit）给合成线程，那么接下来合成线程是怎么工作的呢？

那我们得先来看看什么是视口，你可以参看下图：

![视口](./img/242225112f2a3ec97e736c960b88d972.webp)
...
![图层被划分为图块示意图](./img/bcc7f6983d5ece8e2dd716f431d0e052.webp)
...
![合成线程提交图块给栅格化线程池](./img/d8d77356211e12b47bb9f508e2db8520.webp)

通常，栅格化过程都会使用 GPU 来加速生成，使用 GPU 生成位图的过程叫快速栅格化，或者 GPU 栅格化，生成的位图被保存在 GPU 内存中。

相信你还记得，GPU 操作是运行在 GPU 进程中，如果栅格化操作使用了 GPU，那么最终生成位图的操作是在 GPU 中完成的，这就涉及到了跨进程操作。具体形式你可以参考下图：

![GPU 栅格化](./img/a8d954cd8e4722ee03d14afaa14c3987.webp)
...
![完整的渲染流水线示意图](./img/975fcbf7f83cc20d216f3d68a85d0f37.webp)
...
![更新元素的几何属性](./img/b3ed565230fe4f5c1886304a8ff754e5.webp)
...
![更新元素背景](./img/3c1b7310648cccbf6aa4a42ad0202b03.webp)
...
![避开重排和重绘](./img/024bf6c83b8146d267f476555d953a2c.webp)
